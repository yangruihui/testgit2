$(function() {
	...
	var jobGroupTable = $("#jobgroup_list").dataTable({
		...
		"columns": [
			...
			//数据库配置信息
			{
				"data": 'databaseParamList',
				"width":'10%',
				"visible" : true,
				"render": function ( data, type, row ) {
					return row.databaseParamList
						?'<a class="show_databaseParamList" href="javascript:;" _id="'+ row.id +'" >'
						+ I18n.system_show +'</a>'
						:I18n.system_empty;
				}
			},
	...
	//数据库配置信息
	$("#jobgroup_list").on('click', '.show_databaseParamList',function() {
		var id = $(this).attr("_id");
		var row = tableData['key'+id];

		var html = '<div>';
		if (row.databaseParamList) {
			for (var index in row.databaseParamList) {
				html += '<span class="badge bg-green" >' + row.databaseParamList[index] + '</span><br>';
			}
		}
		html += '</div>';
	...
	//执行器类型
	$("#addModal input[name=executorType], #updateModal input[name=executorType]").click(function(){
		var executorType = $(this).val();
		var $databaseParam = $(this).parents("form").find("textarea[name=databaseParam]");
		if (executorType == 0) {
			$databaseParam.css("background-color", "#eee");	// 系统定时任务
			$databaseParam.attr("readonly","readonly");
			$databaseParam.val("");
		} else {
			$databaseParam.css("background-color", "white");
			$databaseParam.removeAttr("readonly");
		}
	});
	...
	$("#jobgroup_list").on('click', '.opt_edit',function() {
		...
		//数据库配置信息
		$("#updateModal .form textarea[name='databaseParam']").val( row.databaseParam );
		//执行器类型
		$("#updateModal .from input[name='executorType']").removeAttr('checked');
		$("#updateModal .form input[name='executorType'][value='"+row.executorType+"']").click();
